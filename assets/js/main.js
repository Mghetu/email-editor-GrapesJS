...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    var editor = grapesjs.init({
      ...
      fromElement: false,
      height: '100%',
      // Persist data in localStorage so that reloads keep the user’s work.
      storageManager: {
        type: 'local',
        autosave: true,
        autoload: true,
      ...
      // Append the Block Manager and Layer Manager into our custom sidebar.
      blockManager: {
...
<mjml>
  ...
  <mj-body background-color="#ffffff">
    ...
    editor.on('load', function () {
      ...
      // Hide the default block and layer icons from the right panel.
      // When using our custom sidebar, the views panel still contains
      // buttons to open Blocks and Layers.  Remove them to avoid
      // confusion and duplication.  The IDs correspond to the
      // default commands registered by GrapesJS.  If you upgrade
      // GrapesJS these IDs may change; refer to the documentation for
      // the current names (eg. 'open-blocks', 'open-layers').
      var pn = editor.Panels;
      try {
        pn.removeButton('views', 'open-blocks');
        pn.removeButton('views', 'open-layers');
      } catch (e) {
        // Silently ignore if buttons don’t exist.
      }

      // Automatically open the Block and Layer managers inside our custom
      // sidebar.  Without explicitly running these commands, the
      // managers remain hidden until a button is clicked.  Running
      // commands here ensures the UI is visible immediately on load.
      try {
        editor.runCommand('open-blocks');
        editor.runCommand('open-layers');
      } catch (err) {
        // If commands are not available, ignore the error.
      }
    });
    ...
    editor.on('load', function () {
      ...
      /*
       * Override MJML’s default mobile column stacking inside mj-group.
       *
       * MJML renders columns as tables with classes like `mj-column-per-50` and
       * applies a `width: 100%` rule to those classes at small screen sizes,
       * causing columns to stack.  When columns are wrapped in an `mj-group`
       * element, MJML normally adds extra CSS to keep the columns inline by
       * resetting their width to the original percentage and max‑width.  The
       * GrapesJS preview does not include this second rule, so columns inside
       * mj‑group still stack on mobile in the canvas.  To emulate the real MJML
       * output, inject a media query that restores the original widths for
       * group columns and also sets their max‑width accordingly.  The
       * `display` property is set to `table-cell` to ensure inline behaviour.
       */
      // NOTE: GrapesJS does not currently mirror MJML’s group behaviour in its
      // built‑in responsive preview.  MJML prevents columns inside an
      // `mj-group` from stacking on mobile by injecting additional CSS at
      // compile time.  The grapesjs‑mjml plugin omits that rule, so in the
      // canvas mobile view group columns will appear stacked even though
      // they render side by side in the final exported email.  If you wish
      // to experiment with overriding this behaviour, you can append a
      // <style> element to the iframe’s head here with your own media
      // queries.  See the project README for details.
    });
  });
})();